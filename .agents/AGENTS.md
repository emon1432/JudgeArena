...
8. **No Autonomous Git Operations**: The AI Assistant MUST NEVER run `git add`, `git commit`, `git push`, `git checkout`, `git merge`, or any state-modifying Git commands. All version control staging, committing, branching, and pushing will be performed manually by the user. Read-only commands (e.g., `git status`, `git diff`) may be used only when necessary for inspection.
9. **Adaptive Learning & Continuous Knowledge Base Synchronization**: The AI Assistant MUST be an active, adaptive learner. Whenever any strategic, architectural, caching, or synchronization decision is modified, added, or retired (e.g., Google Drive standings caching, dynamic platform registry, removal of 50-chunk limits), the AI Assistant MUST immediately update the corresponding `.agents/*/*.md` documentation files within the same session. Never leave the project's knowledge base stale or out of sync with actual code reality.
