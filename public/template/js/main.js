...
function getThemeColors() {
  ...
  return {
    ...
    gridColor: isDark ? "rgba(255, 255, 255, 0.07)" : "rgba(15, 23, 42, 0.08)",
    blue: "#3b82f6",
    purple: "#8b5cf6",
    green: "#22c55e",
    red: "#ef4444",
        ...
        datasets: [
          {
            label: "Codeforces",
            data: [
              1200, 1340, 1420, 1390, 1560, 1680, 1620, 1790, 1850, 1910, 1880,
              1964,
            ],
          ...
          },
          {
            label: "LeetCode",
            data: [
              1400, 1510, 1630, 1720, 1810, 1890, 1940, 2010, 2080, 2120, 2100,
              2140,
            ],
          ...
          },
          {
            label: "AtCoder",
            data: [
              980, 1050, 1140, 1220, 1310, 1380, 1450, 1490, 1510, 1530, 1520,
              1542,
            ],
          ...
          },
          {
            ...
            data: [
              1190, 1300, 1390, 1440, 1560, 1650, 1670, 1760, 1810, 1850, 1830,
              1882,
            ],
          ...
          tooltip: {
            ...
            callbacks: {
              label: (context) => {
                ...
                const pct = ((val / totalPlatform) * 100).toFixed(1);
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            ...
            cornerRadius: 8,
            callbacks: {
              label: (context) => ` ${context.parsed}% of submissions`,
            },
          },
        },
      },
...
function initPlatformTableControls() {
  ...
  const tableRows = document.querySelectorAll(".saas-directory-table tbody tr");
    ...
    pill.addEventListener("click", () => {
      ...
      tableRows.forEach((row) => {
        if (filterType === "all") {
          ...
          row.style.display = !isNaN(rating) && rating >= 2000 ? "" : "none";
    ...
    opt.addEventListener("click", (e) => {
      ...
      rowsArray.sort((a, b) => {
        if (sortType === "name") {
          ...
          const ratA =
            parseInt(a.children[2].textContent.replace(/,/g, ""), 10) || 0;
          const ratB =
            parseInt(b.children[2].textContent.replace(/,/g, ""), 10) || 0;
          return ratB - ratA;
        } else if (sortType === "solved") {
          const solA =
            parseInt(a.children[4].textContent.replace(/,/g, ""), 10) || 0;
          const solB =
            parseInt(b.children[4].textContent.replace(/,/g, ""), 10) || 0;
          return solB - solA;
        } else {
          return 0;
        }
...
/* ==========================================================================
   5. SUPPORTED PLATFORMS DIRECTORY INTERACTIVITY & PAGINATION ENGINE
   ========================================================================== */

/**
 * Global state for Platforms Directory filtering, sorting & pagination.
 */
let currentPlatformCategory = "all";
let currentPlatformSort = "popular";
let currentPlatformsPage = 1;
let platformsPageSize = 10;
let activePlatformModalId = "codeforces";

/**
 * Initializes Platforms Directory handlers when DOM is ready.
 */
document.addEventListener("DOMContentLoaded", () => {
  const tableBody = document.getElementById("platforms-table-tbody");
  if (!tableBody) return; // Exit if not on platforms.html page

  applyPlatformsFilters(true);
});

/**
 * Filters static HTML platform table rows based on user selections:
 * - Search query (name, code, or domain)
 * - Category tab (all, global, olympiad, specialized)
 * - Sync capability select (all, full, rating, solved)
 * - Connection status select (all, connected, available)
 *
 * @param {boolean} resetPage - Reset to page 1 if true
 */
function applyPlatformsFilters(resetPage = true) {
  const tableBody = document.getElementById("platforms-table-tbody");
  if (!tableBody) return;

  if (resetPage) {
    currentPlatformsPage = 1;
  }

  const rows = Array.from(tableBody.querySelectorAll("tr"));
  const searchQuery = (
    document.getElementById("platforms-directory-search")?.value || ""
  )
    .toLowerCase()
    .trim();
  const selectedSync =
    document.getElementById("platforms-sync-select")?.value || "all";
  const selectedStatus =
    document.getElementById("platforms-status-select")?.value || "all";

  // 1. Filter matching rows
  const matchingRows = [];
  rows.forEach((row) => {
    const pName = (row.getAttribute("data-name") || "").toLowerCase();
    const pCode = (row.getAttribute("data-code") || "").toLowerCase();
    const pCategory = row.getAttribute("data-category") || "";
    const pSync = row.getAttribute("data-sync") || "";
    const pStatus = row.getAttribute("data-status") || "";
    ...
    // Check category tab
    const matchCategory =
      currentPlatformCategory === "all" ||
      pCategory === currentPlatformCategory;

    // Check sync capability
    const matchSync = selectedSync === "all" || pSync === selectedSync;

    // Check connection status
    const matchStatus = selectedStatus === "all" || pStatus === selectedStatus;

    if (matchSearch && matchCategory && matchSync && matchStatus) {
      matchingRows.push(row);
    }
  });

  // 2. Sort matching rows
  sortPlatformRowsArray(matchingRows, currentPlatformSort);
  ...
  // 3. Paginate matching rows
  const totalItems = matchingRows.length;
  const totalPages = Math.ceil(totalItems / platformsPageSize) || 1;
  if (currentPlatformsPage > totalPages) currentPlatformsPage = totalPages;

  const startIndex = (currentPlatformsPage - 1) * platformsPageSize;
  const endIndex = Math.min(startIndex + platformsPageSize, totalItems);
  ...
  updatePlatformsPaginationControls(
    startIndex,
    endIndex,
    totalItems,
    totalPages,
...
/**
 * Sorts array of platform row DOM elements in-memory.
 */
function sortPlatformRowsArray(rowsArray, sortVal) {
  rowsArray.sort((a, b) => {
    const usersA = parseInt(a.getAttribute("data-users") || "0", 10);
    const usersB = parseInt(b.getAttribute("data-users") || "0", 10);
    const nameA = (a.getAttribute("data-name") || "").toLowerCase();
    const nameB = (b.getAttribute("data-name") || "").toLowerCase();

    if (sortVal === "popular" || sortVal === "users-desc") {
...
/**
 * Triggers category tab filter.
 * @param {string} category - 'all' | 'global' | 'olympiad' | 'specialized'
 * @param {HTMLElement} btn - Clicked pill button
 */
function filterPlatformsByCategory(category, btn) {
  currentPlatformCategory = category;

  const pills = document.querySelectorAll("#platform-category-pills .nav-link");
  pills.forEach((p) => p.classList.remove("active"));
  if (btn) btn.classList.add("active");
...
/**
 * Triggers sorting for Platforms Directory.
 * @param {string} sortVal - 'popular' | 'name-asc' | 'users-desc'
 */
function sortPlatformsList(sortVal) {
  currentPlatformSort = sortVal;
  applyPlatformsFilters(false);
}

/**
 * Renders pagination info text and page buttons for Platforms Directory.
 */
function updatePlatformsPaginationControls(
  startIndex,
  endIndex,
  totalItems,
  totalPages,
) {
  const info = document.getElementById("platforms-pagination-info");
  if (info) {
    if (totalItems === 0) {
      info.textContent = "Showing 0 of 0 platforms";
    } else {
      info.textContent = `Showing ${startIndex + 1}-${endIndex} of ${totalItems} platforms`;
    }
  }

  const controls = document.getElementById("platforms-pagination-controls");
  if (!controls) return;
  controls.innerHTML = "";
  ...
  // Previous button
  const prevLi = document.createElement("li");
  prevLi.className = `page-item ${currentPlatformsPage === 1 ? "disabled" : ""}`;
  prevLi.innerHTML = `
    <button class="page-link py-1 px-2 rounded-2" onclick="changePlatformsPage(${currentPlatformsPage - 1})" aria-label="Previous">
      <span aria-hidden="true">&laquo;</span>
    </button>
  `;
  controls.appendChild(prevLi);
  ...
  for (let i = 1; i <= totalPages; i++) {
    const li = document.createElement("li");
    li.className = `page-item ${currentPlatformsPage === i ? "active" : ""}`;
    li.innerHTML = `
      <button class="page-link py-1 px-2.5 rounded-2 fw-semibold" onclick="changePlatformsPage(${i})">${i}</button>
    `;
    controls.appendChild(li);
  }
  ...
  // Next button
  const nextLi = document.createElement("li");
  nextLi.className = `page-item ${currentPlatformsPage === totalPages ? "disabled" : ""}`;
  nextLi.innerHTML = `
    <button class="page-link py-1 px-2 rounded-2" onclick="changePlatformsPage(${currentPlatformsPage + 1})" aria-label="Next">
      <span aria-hidden="true">&raquo;</span>
    </button>
  `;
  controls.appendChild(nextLi);
...
/**
 * Changes current pagination page for Platforms Directory.
 * @param {number} page - Page number to switch to
 */
function changePlatformsPage(page) {
  currentPlatformsPage = page;
  applyPlatformsFilters(false);
}

/**
 * Changes page size for Platforms Directory.
 * @param {string|number} size - Items per page
 */
function changePlatformsPageSize(size) {
  platformsPageSize = parseInt(size, 10) || 10;
  applyPlatformsFilters(true);
}

/**
 * Opens platform handle connect modal.
 * @param {string} platformId - Platform identifier (codeforces, leetcode, atcoder, etc.)
 */
function openPlatformDetailModal(platformId) {
  activePlatformModalId = platformId;
  ...
  const domains = {
    ...
    beecrowd: "beecrowd.io",
  };

  const modalTitle = document.getElementById("modal-platform-name");
  const modalDomain = document.getElementById("modal-platform-domain");
  const handleInput = document.getElementById("platform-handle-input");

  if (modalTitle) {
    modalTitle.textContent = `Connect ${names[platformId] || platformId}`;
  }
  if (modalDomain) {
    modalDomain.textContent = domains[platformId] || `${platformId}.com`;
  }
  if (handleInput) {
    handleInput.value = "";
    handleInput.placeholder = `e.g. tourist`;
  }

  const modalEl = document.getElementById("platformDetailModal");
...
/**
 * Simulates linking platform handle and saving to profile.
 */
function savePlatformHandle() {
  const input = document.getElementById("platform-handle-input");
  ...
  setTimeout(() => {
    if (btn) {
      btn.disabled = false;
      btn.innerHTML = '<i class="fa-solid fa-link me-1"></i> Link Account';
    }
    ...
    alert(
      `Successfully connected handle @${handle}! Synchronizing contest ratings and solved problems in background.`,
    );
  }, 1000);
}

/**
 * Simulates synchronizing all connected platform handles.
 */
function syncAllPlatforms() {
  ...
  btn.disabled = true;
  if (icon) icon.classList.add("fa-spin");
  if (text) text.textContent = "Syncing All Handles...";

  setTimeout(() => {
    btn.disabled = false;
    if (icon) icon.classList.remove("fa-spin");
    if (text) text.textContent = "Sync All Handles";

    alert(
      "Synchronization completed! All connected handles (Codeforces, LeetCode, AtCoder, CodeChef) are fully updated.",
    );
  }, 1200);
}
...
function resendOTPCode() {
  ...
  alert("A new 6-digit verification code has been sent to your email address!");
  startOTPCountdown();
}
...
/* ==========================================================================
   7. GLOBAL RANKINGS INTERACTIVITY & PAGINATION ENGINE
   ========================================================================== */

/**
 * Global state for Rankings Directory filtering, sorting & pagination.
 */
let currentRankingsCategory = "all";
let currentRankingsSort = "rank-asc";
let currentRankingsPage = 1;
let rankingsPageSize = 10;

/**
 * Initializes Rankings Directory handlers when DOM is ready.
 */
document.addEventListener("DOMContentLoaded", () => {
  const tableBody = document.getElementById("rankings-table-tbody");
  if (!tableBody) return; // Exit if not on rankings.html page

  applyRankingsFilters(true);
});

/**
 * Filters static HTML rankings table rows based on user selections:
 * - Search query (name, handle, country, or university)
 * - Category tab (all, lgm, regional)
 * - Platform dropdown filter
 * - Country dropdown filter
 *
 * @param {boolean} resetPage - Reset to page 1 if true
 */
function applyRankingsFilters(resetPage = true) {
  const tableBody = document.getElementById("rankings-table-tbody");
  if (!tableBody) return;
  ...
  const rows = Array.from(tableBody.querySelectorAll("tr"));
  const searchQuery = (
    document.getElementById("rankings-directory-search")?.value || ""
  )
    .toLowerCase()
    .trim();
  const selectedPlatform =
    document.getElementById("rankings-platform-select")?.value || "all";
  const selectedCountry =
    document.getElementById("rankings-country-select")?.value || "all";

  // 1. Filter matching rows
  const matchingRows = [];
  rows.forEach((row) => {
    const pName = (row.getAttribute("data-name") || "").toLowerCase();
    const pHandle = (row.getAttribute("data-handle") || "").toLowerCase();
    const pCountry = row.getAttribute("data-country") || "";
    const pCategory = row.getAttribute("data-category") || "";
    const textContent = row.textContent.toLowerCase();
    ...
    const matchSearch =
      searchQuery === "" ||
      pName.includes(searchQuery) ||
      pHandle.includes(searchQuery) ||
      textContent.includes(searchQuery);

    // Check category tab
    const matchCategory =
      currentRankingsCategory === "all" ||
      pCategory === currentRankingsCategory;

    // Check country dropdown
    const matchCountry =
      selectedCountry === "all" ||
      pCountry.toLowerCase() === selectedCountry.toLowerCase();

    if (matchSearch && matchCategory && matchCountry) {
      matchingRows.push(row);
    }
  });

  // 2. Sort matching rows
  sortRankingsRowsArray(matchingRows, currentRankingsSort);
  ...
  // 3. Paginate matching rows
  const totalItems = matchingRows.length;
  const totalPages = Math.ceil(totalItems / rankingsPageSize) || 1;
  if (currentRankingsPage > totalPages) currentRankingsPage = totalPages;

  const startIndex = (currentRankingsPage - 1) * rankingsPageSize;
  const endIndex = Math.min(startIndex + rankingsPageSize, totalItems);
  ...
  // Update pagination info & controls
  updateRankingsPaginationControls(
    startIndex,
    endIndex,
    totalItems,
    totalPages,
...
/**
 * Sorts array of rankings row DOM elements in-memory.
 */
function sortRankingsRowsArray(rowsArray, sortVal) {
  rowsArray.sort((a, b) => {
    const rankA = parseInt(a.getAttribute("data-rank") || "999", 10);
    const rankB = parseInt(b.getAttribute("data-rank") || "999", 10);
    const cfA = parseInt(a.getAttribute("data-cf") || "0", 10);
    const cfB = parseInt(b.getAttribute("data-cf") || "0", 10);
    const lcA = parseInt(a.getAttribute("data-lc") || "0", 10);
    const lcB = parseInt(b.getAttribute("data-lc") || "0", 10);
    const solvedA = parseInt(a.getAttribute("data-solved") || "0", 10);
    const solvedB = parseInt(b.getAttribute("data-solved") || "0", 10);

    if (sortVal === "rank-asc") {
      return rankA - rankB;
    } else if (sortVal === "cf-desc") {
      return cfB - cfA;
    } else if (sortVal === "lc-desc") {
      return lcB - lcA;
    } else if (sortVal === "solved-desc") {
...
/**
 * Triggers category tab filter for Rankings Directory.
 * @param {string} category - 'all' | 'lgm' | 'regional'
 * @param {HTMLElement} btn - Clicked pill button
 */
function filterRankingsByCategory(category, btn) {
  currentRankingsCategory = category;

  const pills = document.querySelectorAll("#rankings-category-pills .nav-link");
  pills.forEach((p) => p.classList.remove("active"));
  if (btn) btn.classList.add("active");
...
/**
 * Triggers sorting for Rankings Directory.
 * @param {string} sortVal - 'rank-asc' | 'cf-desc' | 'lc-desc' | 'solved-desc'
 */
function sortRankingsList(sortVal) {
  currentRankingsSort = sortVal;
  applyRankingsFilters(false);
}

/**
 * Renders pagination info text and page buttons for Rankings Directory.
 */
function updateRankingsPaginationControls(
  startIndex,
  endIndex,
  totalItems,
  totalPages,
) {
  const info = document.getElementById("rankings-pagination-info");
  if (info) {
    if (totalItems === 0) {
      info.textContent = "Showing 0 of 0 programmers";
    } else {
      info.textContent = `Showing ${startIndex + 1}-${endIndex} of ${totalItems} programmers`;
    }
  }

  const controls = document.getElementById("rankings-pagination-controls");
  if (!controls) return;
  controls.innerHTML = "";
  ...
  // Previous button
  const prevLi = document.createElement("li");
  prevLi.className = `page-item ${currentRankingsPage === 1 ? "disabled" : ""}`;
  prevLi.innerHTML = `
    <button class="page-link py-1 px-2 rounded-2" onclick="changeRankingsPage(${currentRankingsPage - 1})" aria-label="Previous">
      <span aria-hidden="true">&laquo;</span>
    </button>
  `;
  controls.appendChild(prevLi);
  ...
  for (let i = 1; i <= totalPages; i++) {
    const li = document.createElement("li");
    li.className = `page-item ${currentRankingsPage === i ? "active" : ""}`;
    li.innerHTML = `
      <button class="page-link py-1 px-2.5 rounded-2 fw-semibold" onclick="changeRankingsPage(${i})">${i}</button>
    `;
    controls.appendChild(li);
  }
  ...
  // Next button
  const nextLi = document.createElement("li");
  nextLi.className = `page-item ${currentRankingsPage === totalPages ? "disabled" : ""}`;
  nextLi.innerHTML = `
    <button class="page-link py-1 px-2 rounded-2" onclick="changeRankingsPage(${currentRankingsPage + 1})" aria-label="Next">
      <span aria-hidden="true">&raquo;</span>
    </button>
  `;
  controls.appendChild(nextLi);
...
/**
 * Changes current pagination page for Rankings Directory.
 * @param {number} page - Page number to switch to
 */
...
/**
 * Changes page size for Rankings Directory.
 * @param {string|number} size - Items per page
 */
function changeRankingsPageSize(size) {
  rankingsPageSize = parseInt(size, 10) || 10;
  applyRankingsFilters(true);
}

/**
 * Simulates synchronizing global rankings data across judges.
 */
function syncRankingsData() {
  const btn = document.getElementById("sync-rankings-btn");
  const icon = document.getElementById("sync-rankings-icon");
  const text = document.getElementById("sync-rankings-text");

  if (!btn) return;

  btn.disabled = true;
  if (icon) icon.classList.add("fa-spin");
  if (text) text.textContent = "Syncing Rankings...";

  setTimeout(() => {
    btn.disabled = false;
    if (icon) icon.classList.remove("fa-spin");
    if (text) text.textContent = "Sync Rankings";

    alert(
      "Global rankings synchronized! Standings updated with latest Codeforces & AtCoder rating changes.",
    );
  }, 1200);
}
              ...
              const timer = setInterval(() => {
                ...
                const unit = originalText.includes("M")
                  ...
                  : originalText.includes("K")
                    ? "K"
                    : "";
                targetEl.textContent = currentVal + unit + suffix;
              }, 40);
            }
