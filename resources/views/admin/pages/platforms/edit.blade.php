                ...
                <div class="card-body">
                    <form class="row g-6 common-form" action="{{ route('platforms.update', $platform->id) }}" method="POST"
                        ...
                        <div class="col-md-4 form-control-validation">
                            <label class="form-label" for="short_name">{{ __('Short Name') }}<span class="text-danger">*</span></label>
                            <input type="text" name="short_name" id="short_name" class="form-control"
                                placeholder="{{ __('Enter short name') }}" value="{{ old('short_name', $platform->short_name) }}" required />
                        </div>

                        <div class="col-md-8 form-control-validation">
                            <label class="form-label" for="base_url">{{ __('Base URL') }}<span class="text-danger">*</span></label>
                            <input type="url" name="base_url" id="base_url" class="form-control"
                                placeholder="{{ __('Enter base URL') }}" value="{{ old('base_url', $platform->base_url) }}" required />
                        </div>
                        ...
                        <div class="col-md-6 form-control-validation">
                            ...
                            <select class="form-select" name="status" id="status" required>
                                ...
                                <option value="Maintenance" {{ old('status', $platform->status) === 'Maintenance' ? 'selected' : '' }}>{{ __('Maintenance') }}</option>
                                <option value="Coming Soon" {{ old('status', $platform->status) === 'Coming Soon' ? 'selected' : '' }}>{{ __('Coming Soon') }}</option>
                            </select>
                        </div>

                        <div class="col-12 form-control-validation">
                            <label class="form-label" for="description">{{ __('Description') }}</label>
                            <textarea name="description" id="description" class="form-control" rows="4" placeholder="{{ __('Enter description') }}">{{ old('description', $platform->description) }}</textarea>
                        </div>
                        ...
                        <div class="col-12">
                            ...
                            <div id="credentials-wrapper">
                                @php
                                    $credentials = old('credential_keys') ? array_map(null, old('credential_keys', []), old('credential_values', [])) : [];
                                ...
                                @if (count($credentials) > 0)
                                    @foreach ($credentials as $item)
                                        <div class="row g-2 credential-item mb-2">
                                            <div class="col-md-4">
                                                <input type="text" name="credential_keys[]" class="form-control" value="{{ $item[0] ?? '' }}" placeholder="Key (example: api_key)">
                                            </div>

                                            <div class="col-md-6">
                                                <input type="text" name="credential_values[]" class="form-control" value="{{ $item[1] ?? '' }}" placeholder="Value">
                                            </div>
