...
class ImportAtCoderSubmissionsTest extends TestCase
{
    public function test_atcoder_submission_import_claims_and_syncs_each_contest(): void
    {
        ...
        $problem = new class extends Problem {
            public $id = 30;
            public $platform_id = 1;
            public $platform_problem_id = 'abc123_a';
        };

        $submission = new SubmissionDTO(
            platform: 'atcoder',
            platformSubmissionId: '321',
            problemPlatformId: 'abc123_a',
        ...
        $this->mock(AtCoderAdapter::class, function ($mock) use ($submission): void {
        ...
        $this->assertSame('AC', $submissionQuery->values['verdict']);
        $this->assertSame('SAMPLES', $submissionQuery->values['metadata']['testset']);
